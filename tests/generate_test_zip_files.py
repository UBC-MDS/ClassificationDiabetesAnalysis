...
with zipfile.ZipFile('tests/files_txt_csv.zip', 'w', zipfile.ZIP_DEFLATED) as zipf:
    zipf.write('test1.txt')
    zipf.write('test2.csv')

# Case 2 - Create a zip file containing 'test1.txt', test2.csv and 'subdir/test2.txt'
with zipfile.ZipFile('tests/files_txt_subdir.zip', 'w', zipfile.ZIP_DEFLATED) as zipf:
    zipf.write('test1.txt')
    zipf.write('test2.csv')
    zipf.write('subdir/test3.txt')

# Case 3 - Create an empty zip file
with zipfile.ZipFile('tests/empty.zip', 'w', zipfile.ZIP_DEFLATED) as zipf:
    pass
