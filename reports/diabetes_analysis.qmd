...
This study sought to answer the question: Can logistic regression effectively predict diabetes in Pima Indian women using clinical features such as glucose levels, BMI, and pregnancies? 
To address this, the logistic regression model was evaluated and achieved an accuracy of `{python} f"{accuracy_df['accuracy'].values[0] * 100:.0f}%"` accuracy on the test set, 
outperforming the baseline dummy classifier's accuracy of `{python} f"{round(mean_cv_score['mean_cv_score'].values[0],2) * 100:.0f}%"`. 
...
This study aims to address the research question: Can logistic regression accurately predict diabetes in Pima Indian women using clinical features such as glucose levels, BMI, and pregnancies? 
To this end, the objective of this report is to evaluate the predictive performance of logistic regression (LR), a supervised machine learning model, in diagnosing diabetes. 
The analysis is conducted using a real-world dataset comprising clinical data from Pima Indian women aged 21 and older.
...
The dataset offers comprehensive features for machine learning analysis. The Pima Indians Diabetes dataset is not balanced, due to a noticeable disparity between the number of diabetic and non-diabetic patients. Out of the 768 total observations, 268 patients are diagnosed with diabetes, while 500 are not. This imbalance between classes can pose challenges for classification models if not properly addressed. The classification model used can potentially become biased toward predicting the majority class (non-diabetic patients) without proper using a proper scoring metric such as the f1 score and accounting for this imbalance in the models hyper-parameters.
...
with the accuracy score serving as the classification metric. Optimization was done for the regularization strength parameter `C` of the Logistic Regression model using a log-uniform distribution between \(1 \times 10^{-5}\) and \(1 \times 10^{5}\). This approach is necessary because the `C` parameter controls the trade-off between model complexity and fitting the training data, and exploring a wide range of values helps identify the optimal balance for better model performance and generalization.. All variables were standardized just before model fitting. 
